...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="owl:Class">
        ...
        <xsl:if test="not(preceding::*/@IRI = $IRI)">
            ...
            <xsl:choose>
                <xsl:when test="ends-with($annotationRdfsLabel,'Abbreviations') or
                                ends-with($annotationRdfsLabel,'Constraints') or
                                ends-with($annotationRdfsLabel,'OperationalPhases') or
                                ends-with($annotationRdfsLabel,'References') or 
                                ends-with($annotationRdfsLabel,'Risk Element (RE)')">
                        <!-- ignore special case -->
                </xsl:when>
