...
;;;;;;;;;;;;;;;;;;;;;;Algorithmic All Paths by Breadth First Exhaustive Search;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;Proof of Equivalence of Human-directed and Algorithmic All-paths Search;;;;;;;;;;;;;;;;;;;;
...
(defun abbreviated-scenario-element ()
  (let* ((element (pop *mission-execution-scenario*))
         (symbol (convert-path-element-to-symbol element)))
    (list element symbol)))

(defun increment-abbreviated-scenario ()
  (push (second (abbreviated-scenario-element)) *abbreviated-scenario*))

(defun advance-abbreviated-scenario ()
  (list (increment-abbreviated-scenario) *mission-execution-scenario*))

(defun abbreviate-scenario ()
  (dotimes (i 13) (advance-abbreviated-scenario))
  (reverse (remove nil *abbreviated-scenario*)))

(defun reset ()
  (setf *mission-execution-scenario* (first *all-algorithmic-paths*) *abbreviated-scenario* nil))
